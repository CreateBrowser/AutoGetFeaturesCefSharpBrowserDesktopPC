    ...
    /// string UnfilteredLinkUrl { get; }
        ...
        void IContextMenuHandler.OnBeforeContextMenu(IWebBrowser browserControl, IBrowser browser, IFrame frame, IContextMenuParams parameters, IMenuModel model)
        {
           ...
           // for (int i = 0; i < model.Count; i++) if ((int)model.GetCommandIdAt(i) == 234)
            ...
            if (parameters.LinkUrl != "")
            {
              ...
              //  model.AddSeparator();
                ///   model.AddItem((CefMenuCommand)source, "source");
                /// 
            ...
            // model.AddItem((CefMenuCommand)Good, "Save Good");
        ...
        bool IContextMenuHandler.OnContextMenuCommand(IWebBrowser browserControl, IBrowser browser, IFrame frame, IContextMenuParams parameters, CefMenuCommand commandId, CefEventFlags eventFlags)
        {
            ...
            if (commandId == (CefMenuCommand)5010)
            {
                ...
                dialog.Filter = "Png image (*.png)|*.png|ICO image (*.ico)|*.ico|Gif Image (*.gif)|*.gif|JPEG image (*.jpg)|*.jpg|SVG image (*.svg)|*.svg"  /* "|All File (*.*)|*.*" */;
            ...
            */
            ...
            if ((int)commandId == source)
            {
               /// Clipboard.SetText(parameters.LinkUrl);
                /// browser.MainFrame.EvaluateScriptAsync(@"document.getElementsByTagName ('html')[0].innerHTML").ToString();
